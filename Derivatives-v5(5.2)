...
while exprinp!="stop":
    ...
    temp_ans=list(exprecdev(exprinp).replace("()",""))

    #excess bracket removing - main
    stack,i=[],0
    while i<len(temp_ans):
        if temp_ans[i] not in ("(",")"): i+=1
        elif temp_ans[i]=="(": stack.append(i); i+=1
        else:
            s,e=stack[-1]-1,i+1
            stack.pop()
            while i>=0 and e<len(temp_ans) and temp_ans[s]=="(" and temp_ans[e]==")":
                temp_ans[s],temp_ans[e]="!","!"
                s,e=s-1,e+1
            i=e
    op="".join(ch for ch in temp_ans if ch!="!")
    print(op)
